...
if not _SUPABASE_URL or not _SUPABASE_KEY:
    import streamlit as st
    st.error("❌ Missing environment variables: SUPABASE_URL or SUPABASE_KEY is not set on this server.")
    st.info(f"DEBUG — URL set: {bool(_SUPABASE_URL)} | KEY set: {bool(_SUPABASE_KEY)} | KEY prefix: '{_SUPABASE_KEY[:10] if _SUPABASE_KEY else 'EMPTY'}'")
    st.stop()
